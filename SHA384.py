...
class SHA384:
    ...
    H_0 = [int("cbbb9d5dc1059ed8",16), int("629a292a367cd507",16), int("9159015a3070dd17",16), int("152fecd8f70e5939",16), int("67332667ffc00b31",16), int("8eb44a8768581511",16), int("db0c2e0d64f98fa7",16), int("47b5481dbefa4fa4",16)]
    ...
    def __ROTR(x,n):
        if(n<0 or n>=SHA384.w):
            exit("Invalid length for n")
        return SHA384.__SHR(x,n)|SHA384.__SHL(x,SHA384.w-n)

    def __big_sigma_512_0(x):
        rotr28 = SHA384.__ROTR(x,28)
        rotr34 = SHA384.__ROTR(x,34)
        rotr39 = SHA384.__ROTR(x,39)
        return rotr28^rotr34^rotr39

    def __big_sigma_512_1(x):
        rotr14 = SHA384.__ROTR(x,14)
        rotr18 = SHA384.__ROTR(x,18)
        rotr41 = SHA384.__ROTR(x,41)
        return rotr14^rotr18^rotr41

    def __small_sigma_512_0(x):
        rotr1 = SHA384.__ROTR(x,1)
        rotr8 = SHA384.__ROTR(x,8)
        shr7 = SHA384.__SHR(x,7)
        return rotr1^rotr8^shr7

    def __small_sigma_512_1(x):
        rotr19 = (SHA384.__ROTR(x,19))
        rotr61 = SHA384.__ROTR(x,61)
        shr6 = SHA384.__SHR(x,6)
    ...
    def __get_word_block(block):
        size_block = 1024
        words = []
        for i in range(int(size_block/SHA384.w)):
            words.append(int(block[i*SHA384.w:(i+1)*SHA384.w],2))
        return words
    def __get_all_words(blocks):
        size_block = 1024
        words = np.array([[]])
        for i in range(int(len(blocks)/size_block)):
            for j in range(int(size_block/SHA384.w)):
                words=[i][j*SHA384.w:(1+j)*SHA384.w]
        return words
    def __preprocessing(M, initial_state=None, previous_len_bits = 0):
        ...
        if(initial_state == None):
            ...
            H_0 = initial_state
        padded_M = SHA384.__padding(M_bits,previous_len_bits)
        parsed_padded_M = SHA384.__parsing(padded_M)
        return H_0,parsed_padded_M

    #HASH COMPUTATION
    def hash_computation(M,initial_state=None, previous_len_bits = 0):
        H,parsed_padded_M = SHA384.__preprocessing(M,initial_state,previous_len_bits)
        N = len(parsed_padded_M)
        for i in range(0,N):
            W = SHA384.__get_word_block(parsed_padded_M[i])
            for t in range(16,80):
                W.append((SHA384.__small_sigma_512_1(W[t-2]) + W[t-7]+ SHA384.__small_sigma_512_0(W[t-15]) + W[t-16]) & 0xFFFFFFFFFFFFFFFF)
            a = H[0]
            b = H[1]
            c = H[2]
            d = H[3]
            ...
            h = H[7]
            for t in range(80):
                T_1 = (h + SHA384.__big_sigma_512_1(e) + SHA384.__Ch(e,f,g)+ SHA384.K[t] + W[t]) & 0xFFFFFFFFFFFFFFFF
                T_2 = (SHA384.__big_sigma_512_0(a)+ SHA384.__Maj(a,b,c)) & 0xFFFFFFFFFFFFFFFF
                h = g
                g = f
                f = e
        ...
        output_hex = f"{H[0]:016x}"+f"{H[1]:016x}"+f"{H[2]:016x}"+f"{H[3]:016x}"+f"{H[4]:016x}"+f"{H[5]:016x}"
...
if __name__ == "__main__":
    ...
    for _ in range(100):
        ...
        my = SHA384.hash_computation(test_data)
        real_hex = hashlib.sha384(test_data).hexdigest()
