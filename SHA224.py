...
class SHA224:
    ...
    H_0 = [int("c1059ed8 ",16), int("367cd507",16), int("3070dd17",16), int("f70e5939",16), int("ffc00b31",16), int("68581511",16), int("64f98fa7",16), int("befa4fa4",16)]
    ...
    def __ROTR(x,n):
        if(n<0 or n>=SHA224.w):
            exit("Invalid length for n")
        return SHA224.__SHR(x,n)|SHA224.__SHL(x,SHA224.w-n)

    def __big_sigma_256_0(x):
        rotr2 = SHA224.__ROTR(x,2)
        rotr13 = SHA224.__ROTR(x,13)
        rotr22 = SHA224.__ROTR(x,22)
        return rotr2^rotr13^rotr22

    def __big_sigma_256_1(x):
        rotr6 = SHA224.__ROTR(x,6)
        rotr11 = SHA224.__ROTR(x,11)
        rotr25 = SHA224.__ROTR(x,25)
        return rotr6^rotr11^rotr25

    def __small_sigma_256_0(x):
        rotr7 = SHA224.__ROTR(x,7)
        rotr18 = SHA224.__ROTR(x,18)
        shr3 = SHA224.__SHR(x,3)
        return rotr7^rotr18^shr3

    def __small_sigma_256_1(x):
        rotr17 = SHA224.__ROTR(x,17)
        rotr19 = SHA224.__ROTR(x,19)
        shr10 = SHA224.__SHR(x,10)
    ...
    def __preprocessing(M, initial_state=None, previous_len_bits = 0):
        ...
        if(initial_state == None):
            ...
            H_0 = initial_state
        padded_M = SHA224.__padding(M_bits,previous_len_bits)
        parsed_padded_M = SHA224.__parsing(padded_M)
    ...
    def hash_computation(M,initial_state=None, previous_len_bits = 0):
        ...
        for i in range(0,N):
            W = SHA224.__get_word_block(parsed_padded_M[i])
            for t in range(16,64):
                W.append((SHA224.__small_sigma_256_1(W[t-2]) + W[t-7]+ SHA224.__small_sigma_256_0(W[t-15]) + W[t-16]) & 0xFFFFFFFF)
            a = H[0]
            b = H[1]
            c = H[2]
            d = H[3]
            ...
            h = H[7]
            for t in range(64):
                T_1 = (h + SHA224.__big_sigma_256_1(e) + SHA224.__Ch(e,f,g)+ SHA224.K[t] + W[t])& 0xFFFFFFFF
                T_2 = (SHA224.__big_sigma_256_0(a)+ SHA224.__Maj(a,b,c)) & 0xFFFFFFFF
                h = g
                g = f
                f = e
                e = (d + T_1)& 0xFFFFFFFF
