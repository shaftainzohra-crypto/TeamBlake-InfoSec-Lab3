...
class SHA512_224:
    ...
    H_0 = [int("8C3D37C819544DA2",16), int("73E1996689DCD4D6",16), int("1DFAB7AE32FF9C82",16), int("679DD514582F9FCF",16), int("0F6D2B697BD44DA8",16), int("77E36F7304C48942",16), int("3F9D85A86A1D36C8",16), int("1112E6AD91D692A1",16)]
    ...
    def __ROTR(x,n):
        if(n<0 or n>=SHA512_224.w):
            exit("Invalid length for n")
        return SHA512_224.__SHR(x,n)|SHA512_224.__SHL(x,SHA512_224.w-n)

    def __big_sigma_512_0(x):
        rotr28 = SHA512_224.__ROTR(x,28)
        rotr34 = SHA512_224.__ROTR(x,34)
        rotr39 = SHA512_224.__ROTR(x,39)
        return rotr28^rotr34^rotr39

    def __big_sigma_512_1(x):
        rotr14 = SHA512_224.__ROTR(x,14)
        rotr18 = SHA512_224.__ROTR(x,18)
        rotr41 = SHA512_224.__ROTR(x,41)
        return rotr14^rotr18^rotr41

    def __small_sigma_512_0(x):
        rotr1 = SHA512_224.__ROTR(x,1)
        rotr8 = SHA512_224.__ROTR(x,8)
        shr7 = SHA512_224.__SHR(x,7)
        return rotr1^rotr8^shr7

    def __small_sigma_512_1(x):
        rotr19 = (SHA512_224.__ROTR(x,19))
        rotr61 = SHA512_224.__ROTR(x,61)
        shr6 = SHA512_224.__SHR(x,6)
    ...
    def __get_word_block(block):
        size_block = 1024
        words = []
        for i in range(int(size_block/SHA512_224.w)):
            words.append(int(block[i*SHA512_224.w:(i+1)*SHA512_224.w],2))
        return words
    def __get_all_words(blocks):
        size_block = 1024
        words = np.array([[]])
        for i in range(int(len(blocks)/size_block)):
            for j in range(int(size_block/SHA512_224.w)):
                words=[i][j*SHA512_224.w:(1+j)*SHA512_224.w]
        return words
    def __preprocessing(M, initial_state=None, previous_len_bits = 0):
        ...
        if(initial_state == None):
            H_0 = SHA512_224.H_0.copy()
        else:
            H_0 = initial_state
        padded_M = SHA512_224.__padding(M_bits,previous_len_bits)
        parsed_padded_M = SHA512_224.__parsing(padded_M)
        return H_0,parsed_padded_M

    #HASH COMPUTATION
    def hash_computation(M,initial_state=None, previous_len_bits = 0):
        H,parsed_padded_M = SHA512_224.__preprocessing(M,initial_state,previous_len_bits)
        N = len(parsed_padded_M)
        for i in range(0,N):
            W = SHA512_224.__get_word_block(parsed_padded_M[i])
            for t in range(16,80):
                W.append((SHA512_224.__small_sigma_512_1(W[t-2]) + W[t-7]+ SHA512_224.__small_sigma_512_0(W[t-15]) + W[t-16]) & 0xFFFFFFFFFFFFFFFF)
            a = H[0]
            b = H[1]
            c = H[2]
            d = H[3]
            ...
            h = H[7]
            for t in range(80):
                T_1 = (h + SHA512_224.__big_sigma_512_1(e) + SHA512_224.__Ch(e,f,g)+ SHA512_224.K[t] + W[t]) & 0xFFFFFFFFFFFFFFFF
                T_2 = (SHA512_224.__big_sigma_512_0(a)+ SHA512_224.__Maj(a,b,c)) & 0xFFFFFFFFFFFFFFFF
                h = g
                g = f
                f = e
...
for _ in range(100):
    ...
    my = SHA512_224.hash_computation(test_data)
    real_hex = hashlib.new('sha512_224', test_data).hexdigest()
